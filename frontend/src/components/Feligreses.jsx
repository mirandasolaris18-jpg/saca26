...
import { fetchFeligreses, createFeligres } from "../services/feligresesService";
...
export default function Feligreses({ onVolver }) {
  ...
  const cargarDatos = async () => {
    try {
      const data = await fetchFeligreses(busqueda);
      
      // Validamos la estructura de la respuesta
      if (Array.isArray(data)) {
        setFeligreses(data);
      } else if (data && Array.isArray(data.feligreses)) {
        setFeligreses(data.feligreses);
      } else if (data && Array.isArray(data.data)) {
        setFeligreses(data.data);
      } else {
        setFeligreses([]);
      }
    } catch (error) {
      console.error("Error obteniendo feligreses:", error);
    }
  };

  const obtenerCiudades = async () => {
    try {
      // Como no pusimos las ciudades en el service aún, lo llamamos directo por ahora
      const resp = await fetch("http://localhost:5000/api/feligreses/ciudades-lista");
      const data = await resp.json();
      if (resp.ok) setCiudades(data);
    } catch (error) {
      console.error("Error conectando al backend para ciudades:", error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      cargarDatos(); // Recargar la tabla tras guardar
    } catch (error) {
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-black/50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* COMBOBOX CON LOS DATOS DE CIUDADES */}
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">
